...
class Redis extends AbstractDriver
{
    ...
    protected function makeKeyString($key, $path = false)
    {
        ...
        //fix quantico
        echo $methodname." ".$pathKey." ".md5($keyString)."<br>";
         return  ($path ? $pathKey :  md5($keyString));
       // return urlrewrite($sitename."-".$methodname."-").($path ? $pathKey :  md5($keyString));
    }
    ...
    FIX QUANTICO
      protected function makeKeyString($key, $path = false)
    {
        $key = \Stash\Utilities::normalizeKeys($key , 'strtolower');
        ...
        return urlrewrite($path ? $pathKey :  ($keyString));
    }
    */
